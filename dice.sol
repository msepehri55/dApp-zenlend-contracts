...
contract DiceRoll {
    ...
    event Rolled(address player, uint256 amount, uint8 guess, uint8 result, bool won);
    ...
    function roll(uint8 guess, uint256 betAmount) external payable nonReentrant {
        if (msg.value != betAmount || betAmount < minBet || betAmount > maxBet) revert InvalidBet();
        require(guess >= 1 && guess <= 6, "Guess 1-6");

        // Ensure worst-case (win) is coverable before rolling
        if (availableBank() < betAmount * 6) revert InsufficientBank();

        uint256 r = _rand();
        uint8 result = uint8(r % 6) + 1;
        bool won = (result == guess);
        uint256 payout = won ? betAmount * 6 : 0;

        _stats[msg.sender].totalBet += betAmount;
        globalTotalBet += betAmount;

        if (won) {
            // guaranteed by pre-check, but keep guard
        ...
        emit Rolled(msg.sender, betAmount, guess, result, won);
    }
